...
/*   Updated: 2022/11/28 21:52:03 by yismaili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	texture[0].img = mlx_xpm_file_to_image(cub->mlx_ptr, cub->drct.north_path, \
			&texture[0].img_width, &texture[0].img_height);
	texture[0].data = (int *)mlx_get_data_addr(texture[0].img, \
			&texture[0].bits_per_pixel, &texture[0].size_line, \
			&texture[0].endian);
	texture[1].img = mlx_xpm_file_to_image(cub->mlx_ptr, cub->drct.south_path, \
			&texture[1].img_width, &texture[1].img_height);
	texture[1].data = (int *)mlx_get_data_addr(texture[1].img, \
			&texture[1].bits_per_pixel, &texture[1].size_line, \
			&texture[1].endian);
	texture[2].img = mlx_xpm_file_to_image(cub->mlx_ptr, cub->drct.east_path, \
			&texture[2].img_width, &texture[2].img_height);
	texture[2].data = (int *)mlx_get_data_addr(texture[2].img, \
			&texture[2].bits_per_pixel, &texture[2].size_line, \
			&texture[2].endian);
	texture[3].img = mlx_xpm_file_to_image(cub->mlx_ptr, cub->drct.west_path, \
			&texture[3].img_width, &texture[3].img_height);
	texture[3].data = (int *)mlx_get_data_addr(texture[3].img, \
			&texture[3].bits_per_pixel, \
			&texture[3].size_line, &texture[3].endian);
	if (!texture[0].img || !texture[1].img || !texture[2].img || \
			!texture[3].img)
		return ;
	if (!texture[0].data || !texture[1].data || !texture[2].data || \
			!texture[3].data)
		return ;
	if (texture[0].img_width != texture[0].img_height || texture[1]. \
			img_width != texture[1].img_height || texture[2]. \
			img_width != texture[2].img_height || texture[3]. \
			img_width != texture[3].img_height)
		return ;	
}
